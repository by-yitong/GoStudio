...
suspend fun install_cmake_from_url(
    ...
    val archive_path = "/home/gostudio/cache/$archive_name"
...
suspend fun uninstall_cmake_tool(
    ...
    val target = if (key.isBlank()) "/home/gostudio/cmake" else "/home/gostudio/cmake/${normalize_cmake_install_name(key)}"
    val command = """
        set -e
        TARGET=${shell_quote(target)}
        case "@D@TARGET" in /home/gostudio/cmake|/home/gostudio/cmake/*) ;; *) echo "错误: 拒绝删除非 CMake 目录: @D@TARGET"; exit 1 ;; esac
        if [ ! -d "@D@TARGET" ]; then
          echo "CMake 未安装: @D@TARGET"
          exit 0
        fi
        if [ "@D@TARGET" = "/home/gostudio/cmake" ]; then
          FOUND=0
          while IFS= read -r candidate; do
            case "@D@candidate" in /home/gostudio/cmake/*) ;; *) echo "错误: 拒绝删除非 CMake 目录: @D@candidate"; exit 1 ;; esac
          ...
          if [ -f "@D@TARGET/bin/cmake" ]; then
            ...
            rm -rf "@D@TARGET"
            rmdir /home/gostudio/cmake 2>/dev/null || true
            echo "CMake 卸载完成"
          else
...
suspend fun uninstall_ndk_tool(
    ...
    val target = installed?.proot_dir ?: "/home/gostudio/ndk/${normalize_ndk_install_name(key)}"
    val command = """
        set -e
        TARGET=${shell_quote(target)}
        case "@D@TARGET" in /home/gostudio/ndk/*) ;; *) echo "错误: 拒绝删除非 NDK 目录: @D@TARGET"; exit 1 ;; esac
        if [ -d "@D@TARGET" ]; then
...
private fun create_cmake_install_command(
    ...
    val script = """
        set -e
        set -o pipefail
        CACHE_DIR=/home/gostudio/cache
        CMAKE_ROOT=/home/gostudio/cmake
...
private suspend fun install_bundled_ninja(
    ...
    if (!cmake_target.startsWith("/home/gostudio/cmake/")) {
        emit_log(on_log, "错误: CMake 目标目录无效: $cmake_target")
        return@withContext false
    }
    val relative_path = cmake_target.removePrefix("/home/gostudio").trimStart('/')
    if (relative_path.isBlank()) {
        emit_log(on_log, "错误: CMake 目标目录无效: $cmake_target")
        return@withContext false
    }
    val target_dir = File(toolchain_runtime_provider.paths().gostudio_dir, relative_path)
...
private fun host_path_to_proot_path(path: String): String {
    ...
    val xcode_path = toolchain_runtime_provider.paths().gostudio_dir.absolutePath.trimEnd('/')
    ...
    return when {
        absolute_path == xcode_path -> "/home/gostudio"
        absolute_path.startsWith("$xcode_path/") -> "/home/gostudio/" + absolute_path.removePrefix("$xcode_path/")
